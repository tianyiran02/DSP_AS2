...
Fir1fixed fir("50HzandDCdriftFilter2.dat",12);
Fir1fixed fir2("MatchFilter2.dat",12);
	...
	while (1) {
        ...
        //			value = comediRecord->lp[n][i]->filter(value);
                    ...
                    if (comediRecord->filterCheckbox->checkState()==Qt::Checked) {
                        ...
                        float scaleCoefficient=2^12;
                        value=value*scaleCoefficient;
                        value=fir.filter(value);
                        value=(float)(value);
                        value=value/scaleCoefficient;
/*
                        value=value*scaleCoefficient;
                        ...
                        if ((value>1) && (pulseMargin == 0))
                        {

                            pulseMargin = 150;
                            pulseRate = (float)1*1000*60/counterTime;

                            if(((int)pulseRate <= 120) && ((int)pulseRate >= 50))
                                 printf("%f\n",pulseRate);
